    ...
    void Update()
    {
        ...
        for (int i = 0; i < pos.Length; i++)
        {
            if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
            {
                ...
                Transform childTransform = imageContent.transform.GetChild(i);
                childTransform.localScale = Vector2.Lerp(imageContent.transform.GetChild(i).localScale, new Vector2(1.2f, 1.2f), 0.1f);
                childTransform.GetComponent<Image>().color = colors[1];
